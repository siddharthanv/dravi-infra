...
import SROPage from "../../components/SROPage/SROPage";
import { useRouter } from "next/router";
import { getSROByName } from "../../registerOfficeDetailsJsonFullList";
...
function SRO() {
  ...
  const sroSlug = router.query.sro;
  const filteredSroName = getSROByName(sroSlug);
  if (!filteredSroName) {
  ...
  return (
    <div>
      ...
      />
      <SROPage
        sroName={filteredSroName.sroName}
        address={filteredSroName.address}
        district={filteredSroName.district}
        email={filteredSroName.email}
        contactNo={filteredSroName.contactNo}
        SRO1={SRO1}
      />
